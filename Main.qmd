...
Reduced grazing, warmer climate, and atmospheric nitrogen deposition are some of the main global change drivers in alpine environments.
They occur simultaneously, interact and impact biodiversity through the same underlying mechanism.
Warming and nitrogen addition affect biodiversity often negatively, while for grazing this depends on grazing intensity and plant productivity. 
...
We studied the single and interactive effects of these three drivers on alpine biodiversity and under which conditions grazing is beneficial for diversity.
For this, we manipulated warmer climate (transplant to lower elevation), nitrogen deposition (fertilizer) and grazing (removal of biomass) along an elevational gradient (alpine and sub-alpine site) and assessed change in vascular plant composition over three years.

At low productivity (alpine site) warming in combination with increased nitrogen decreases diversity, but simulated grazing can mitigate this, especially at nigtrogen addtion above 10 kg N ha^-1^ y^-1^.
...
This study shows that grazing can benefit biodiversity if grazing leads to competitive release, and thus has the potential to be an ecosystem management tool.
...
These global change drivers can occur simultaneously and impact plant communities through the same underlying mechanisms: change in productivity and biomass (@fig-concept).
...
Warmer climate and nitrogen deposition often (in)directly increase nutrient availability (e.g. longer growing season), with a positive effect on plant productivity and biomass.
...
The net outcome for productivity depends on the grazing intensity, grazing history and environmental context (e.g. moisture, temperature) and in long-term grazed systems, this is a non-linear relationship with the highest productivity at intermediate grazing intensity [@mcnaughton1979grazing; @cingolani2005grazing].
...
In productive systems, removal of biomass can therefore release competition and increase diversity (grey area in @fig-concept b and c), but less so in low productive systems [@Austrheim2008-nu; @Bernes2015-yg].
...
Alpine plant communities were exposed to a combination of warmer climate (transplant to lower elevation), increased nitrogen deposition (N addition), and simulated grazing by removing biomass at two different intensities.
...
Simulated grazing increased diversity and evenness, especially the intensive grazing treatment and for nitrogen addition up to 100 kg N ha^-1^ y^-1^.
...
The nitrogen addition levels were unevenly distributed with some very high levels, and we did not expect a linear response along this gradient.
...
This approach means that we used the model that fits the data best.

To test the effect of the three global change drivers on change in plant functional group cover and diversity, we also used a fixed effects linear model with a normal error distribution.
