  ...
  # run linear, log and quadratic model
    dat <- dat |>
      ...
      mutate(
        ...
        glance_linear = map(.x = model_linear,
                            .f = ~ safely(glance)(.x)$result),
        glance_log = map(.x = model_log,
                         .f = ~ safely(glance)(.x)$result),
        glance_quadratic = map(.x = model_quadratic,
                               .f = ~ safely(glance)(.x)$result)
      )
    ...
    dat |>
      ...
      mutate(
        ...
        model_main_quadratic = map(data, ~lm(.response ~ warming + .grazing + poly(Namount_kg_ha_y, 2) + origSiteID,
                                             data = .)),
        # get aic and r squared
        glance_main_linear = map(.x = model_main_linear,
                                   .f = ~ safely(glance)(.x)$result),
        glance_main_log = map(.x = model_main_log,
                                .f = ~ safely(glance)(.x)$result),
        glance_main_quadratic = map(.x = model_main_quadratic,
                                    .f = ~ safely(glance)(.x)$result)
      )
...
run_full_model_v3 <- function(dat,
  ...
  dat <- dat |>
    ...
    mutate(
      ...
      # get aic and r squared
      glance_bio_log = map(.x = model_bio_log,
                           .f = ~ safely(glance)(.x)$result),
      glance_main_linear = map(.x = model_main_linear,
                                  .f = ~ safely(glance)(.x)$result),
      glance_biomain_linear = map(.x = model_biomain_linear,
                                     .f = ~ safely(glance)(.x)$result),
      glance_interaction_linear = map(.x = model_interaction_linear,
                                         .f = ~ safely(glance)(.x)$result),
      glance_biointeraction_linear = map(.x = model_biointeraction_linear,
                                            .f = ~ safely(glance)(.x)$result)
    )
...
run_full_model_v3_origin <- function(dat,
  ...
  dat <- dat |>
    ...
    mutate(
      ...
      glance_main_quadratic = map(.x = model_main_quadratic,
                       .f = ~ safely(glance)(.x)$result),
      glance_biomain_quadratic = map(.x = model_biomain_quadratic,
                             .f = ~ safely(glance)(.x)$result),
      glance_interaction_quadratic = map(.x = model_interaction_quadratic,
                             .f = ~ safely(glance)(.x)$result),
      glance_biointeraction_quadratic = map(.x = model_biointeraction_quadratic,
                             .f = ~ safely(glance)(.x)$result)
    )
}
