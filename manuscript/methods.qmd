...
We ran the analysis first across both sites including site as a co-variate and then separately for the alpine and sub-alpine site. This was done because the two site differed strongly in their productivity and diversity and how they respond to the three global change drivers and we were interested to understand if different drivers play different roles at these sites and if their interactions are more important at one of the sites.
...
To test the relationship between standing biomass and Shannon diversity, we run a fixed effects linear model with a normal error distribution. Diveresity was the response and the interaction of standing biomass and site the predictors. (We also ran it with warming as predictor... should we describe both models or just one of them...?). (Justify, why we ignore the drivers here?).

To test if the three global change drivers affect Shannon diversity through the same mechanisms, i.e. standing biomass, we first tested how important it was to add the interactions of the global change drivers in the model. For this we compared three linear models with Shannon diversity as response and including the following predictors: 1) only the main effects, warming, nitrogen addition and clipping, 2) the main effects and their interactions, and 3) the main effects and standing biomass (log transformed). We ran these models separately for each site and compared them using AIC values. At the alpine site, adding standing biomass or the interactions between the drivers improved the model equally well, by ca. 2.5 AIC units (@tbl-var-explained). However at the sub-alpine site, the best model was the main effects model only. We decided therefore decided that dropping the interactions of the global change drivers from further was justified.
...
To better understand the underlying mechanisms, i.e. change in abundance, colonization and extinctions, for how the global change drivers affect diversity, we compared traits of *lose*r and *winner*. Species were classified as *losers* if they went *extinct* (species that are present pre-treatment but not in the final year and thus go extinct) or *decreased in cover* (species that decrease in cover \> 10% between pre-treatment and the final year). Contrary, species were classified as *winners* if they *colonized* the plots (species that are not present pre-treatment but in the final year and colonize the plot) or *increased* in cover (species that increase in cover \> 10% between pre-treatment and the final year). For each species we compiled the following traits: plant height, Ellenberg light and temperature indicator values. We then calculated the community weighted trait means for losers and winner using the traitstrap package [@telford2021traitstrap; @maitner2023bootstrapping]. Finally, we tested if the global change drivers affect these community weighted trait means for losers and winners using a linear fixed effects model. We ran separate models for each trait as response and the interactions of the three drivers as predictors. The analysis was run separately for each site.

Finally, the effect of grazing on diversity was calculated by the difference in diversity in the medium or intensive grazed plots and control plots. To test the relationship between the three global change drivers and diversity and how this relationship changes with standing biomass, we used a linear regression with a normal error distribution and the interactions of warming and nitrogen addition, and standing biomass in the control plots, as a baseline, as predictors and the effect of grazing on diversity as response. The analysis was done separately by site.
