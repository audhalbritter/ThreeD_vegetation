...
To summarize the environmental affinities across the alpine and sub-alpine plant communities, we performed a principle component analysis (PCA) with scaled and centered variables across both sites using the vegan package [@oksanen2022vegan]. The first two PC axes explained 83.3 % of the variation together, and we therefore did not consider more axes. We then ran the same SEM as described above where the affinity PC axis 1 and 2 replace the diversity indices. Because the joint PCA across sites reflected the sub-alpine site less well compared to the alpine site, we repeated the PCA separately for each site and ran the SEMs again with affinity PC axis from the PCA analysis separated by site (see Supplementary Figures S6, and S7).